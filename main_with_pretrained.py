...
def build_model(lr, l2, flags, activation='sigmoid'):
    ##############
    # BRANCH MODEL
    ##############
...
model, branch_model, head_model = build_model(64e-5, 0, 0)
...
def compute_score(verbose=1):
    ...
    Compute the score matrix by scoring every pictures from the training set against every other picture O(n^2).
    """
    features = branch_model.predict_generator(FeatureGen(train, batch_size=64, verbose=verbose),
    ...
    for start in range(0, features.shape[0], batch):
        ...
        temp_score = head_model.predict_generator(ScoreGen(temp_features, batch_size=4096, verbose=verbose),
                                             max_queue_size=12, workers=6, verbose=0)
...
if isfile('./models/standard.h5'):
    ...
    # epoch -> 10
    make_steps(10, 1000)
    model.save_weights('./models/standard_epoch10.h5', overwrite=True)

    # 解冻卷积基base_model
    model, branch_model, head_model = build_model(64e-5, 0, 1)
